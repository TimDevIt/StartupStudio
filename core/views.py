...
from core.models import Project, ProjectEntry, Team, TeamEntry, Skill, Direction
...
def guest_or_authenticated_required(view_func):
    @wraps(view_func)
...
@guest_or_authenticated_required
...
@method_decorator(guest_or_authenticated_required, name='dispatch')
...
def register_request(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(request=request, template_name="core/register.html", context={"register_form": form, "errors": form.errors})
...
def create_project_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            project.project_skills.set(form.cleaned_data['project_skills'])
...
@permission_required('core.can_enter_projects')
...
def load_skills(request):
    ...
    skills = Skill.objects.filter(direction=direction)
    skills_dict = {skill.pk: skill.skill_name for skill in skills}
